...
let that = this;
  ...
  methods: {
    ...
    delDevHandle(done) {
      this.$confirm('确认删除？')
      .then(_=>{
          this.$http.post("/type/queryTypePid",{id:this.menuForm.id}).then((res)=>{
            if (res.data.length!=0) {
              // console.log(res.data);
              alert("该菜单有子级菜单，不允许删除")
            }else{
              this.$http.post("/type/delTypeHandle",{id:this.menuForm.id}).then((res)=>{
          if(res.succes){
            console.log("删除成功");
          }
        })
    ...
    editBrand() {
      ...
      if(this.operation==0||this.operation==2){
        this.$http.post("/type/addTypeHandle",this.menuForm).then((res)=>{
      if(res.succes){
        console.log(res.data);
      }
     })
      }else if(this.operation==1){
        this.$http.post("/type/editTypeBrand",that.menuForm).then((res) => {
